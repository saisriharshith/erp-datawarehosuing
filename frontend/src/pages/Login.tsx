...
export const Login: React.FC = () => {
  ...
  const handleQuickFill = (role: 'admin' | 'lecturer') => {
    if (role === 'admin') {
      setEmail('admin@college.edu');
      setPassword('Admin@12345');
    } else {
      setEmail('prof.smith@college.edu');
      setPassword('Lecturer@12345');
    }
  };
          ...
          <div className="mt-6 pt-5 border-t border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="px-3 py-2 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-xs font-medium text-slate-300 text-center transition-colors flex items-center justify-center space-x-1.5"
              >
              ...
              <button
                type="button"
                onClick={() => handleQuickFill('lecturer')}
                className="px-3 py-2 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 text-xs font-medium text-slate-300 text-center transition-colors flex items-center justify-center space-x-1.5"
              >
